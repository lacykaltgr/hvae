...
class ImageNetDataSet(_DataSet):
    ...
    def load(self):
        data_params = get_hparams().data_params
        ...
        for idx in range(1, 10):
            file_path = os.path.join(root_path, f'train_data_batch_{idx}')
        ...
        # VAL DATA
        file_path = os.path.join(root_path, f'val_data')
        x, y = load_databatch(file_path)
        val_data = x

        # TEST DATA
        file_path = os.path.join(root_path, f'test_data')
        x, y = load_databatch(file_path)
        test_data = x
